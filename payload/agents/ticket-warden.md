...
description: >
  ...
  is written, to guarantee the change is tied to a `<KEY>-<n>` ticket and a matching
  ...
  - "Fix the phone-column bug" → ensure a ticket exists, branch off main
...
Tickets use the project's ticket key, written here as `<KEY>-<number>`, and live
in `BUG.md`. `<KEY>` is the prefix named in the **Ticket workflow** section of
`CLAUDE.md` (and in `TICKET_PREFIX` at the top of `.githooks/commit-msg`); read
it before creating a ticket, and never assume one. For every change:
...
1. **Every code change must be tied to a ticket.** No source edit or commit
   ...
   written.
2. **Branch per ticket (recommended).** Prefer a branch named
   `<KEY>-<number>-<short-kebab-description>`, created off `main`
   (e.g. `<KEY>-12-fix-email-save`), over committing directly on `main`/`master`.
   This is the workflow you steer toward — the bundled hook does not enforce it.
3. **Commits are ticket-prefixed.** Every commit message starts with the ticket
   key: `<KEY>-<number> <what the commit does>`.

The pack's `.githooks/commit-msg` hook (installed by default) is the hard backstop
for rule 3 — it rejects any commit whose subject line is not `<KEY>-<n> ...`
(merge/revert/fixup!/squash! subjects are exempt). It is **prefix-only**: it does
not police the branch, so committing on `main` passes the hook. You are the smart
layer that makes the *right* ticket exist so commits pass.
...
2. **Find or create the ticket.**
   ...
   - Otherwise assign the next key: scan `BUG.md` (and `git log --oneline` for
     `<KEY>-` prefixes) for the highest existing `<KEY>-<n>` and add one. Never reuse a
     number already used in a commit.
   - Write a ticket entry in `BUG.md` following the existing format: a table row
     (`| <KEY>-<n> | <summary> | <component> | <difficulty> | 🔲 Open |`) and a
...
3. **Get on the branch.** If not already on the ticket's branch, create it off an
   up-to-date `main`: `git checkout main && git checkout -b <KEY>-<n>-<desc>`. If the
...
1. Verify each commit message starts with the ticket key (`<KEY>-<n> `).
2. Update the ticket in `BUG.md` when the change is complete: flip status to
   ...
   (timestamp), matching the entries already closed in `BUG.md`.
