...
2. **Branch per ticket (recommended).** Prefer a branch named
   `SR-<number>-<short-kebab-description>`, created off `main`
   (e.g. `SR-104-fix-email-save`), over committing directly on `main`/`master`.
...
for rule 3 — it rejects any commit whose subject line is not `SR-<n> ...`
